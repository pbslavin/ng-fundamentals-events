...
export class EventService {
    ...
    saveEvent(event) {
      let options = { headers: new HttpHeaders({ 'Content': 'application/json' })}
      return this.http.post<Event>('api/events', event, options)
        .pipe(catchError(this.handleError<Event>('saveEvent')));
    }
    
    // updateEvent no longer needed, because server differentiates between update and new in saveEvent
